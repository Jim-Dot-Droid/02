...
def get_under_average():
    """Return average under count for last WINDOW rounds"""
    history = load_history()
    if len(history) < WINDOW:
        return 0
    recent = np.array(history[-WINDOW:])
    return int(np.sum(recent < 2.0) / len(recent) * WINDOW)

def update_martingale_balance(actual):
    """Custom martingale: 0.01 → 0.02 → 0.05 → stop until under_avg >= 8"""
    balance = get_martingale_balance()
    bet = get_martingale_bet()

    under_avg = get_under_average()
    if bet == 0 and under_avg < 8:
        # Still on break
        return

    if actual > 2.0:  # Win
        balance += bet
        bet = MARTI_BASE_BET  # Reset sequence
    else:  # Loss
        balance -= bet
...
# --- Streamlit App ---
def main():
    st.title("Crash Predictor — Under Count Strategy with Martingale 0.01→0.02→0.05")
    ...
    st.caption("Martingale bets sequence: 0.01 → 0.02 → 0.05 → stop until under average ≥ 8. Bets only when 'Above' is predicted and result recorded.")
